...
boolean settingsRead() {
  ...
  // ... (Todo el inicio igual hasta llegar a la parte de forzar configuración)

  // --- CONFIGURACIÓN FORZADA (Tus datos reales corregidos) ---
  strlcpy(wifi_ssid, "Redmi Note 11S", sizeof(wifi_ssid));
  strlcpy(wifi_password, "holamundo", sizeof(wifi_password));
  strlcpy(mqtt_server, "10.110.191.56", sizeof(mqtt_server));
  strlcpy(mqtt_user, "plcuser", sizeof(mqtt_user));
  strlcpy(mqtt_password, "plc", sizeof(mqtt_password));

  // CAMBIAMOS EL ID PARA QUE NO DIGA "ALREADY CONNECTED"
  strlcpy(mqtt_cloud_id, "ESP32_S3_ANGEL_VALOR", sizeof(mqtt_cloud_id));

  // MUY IMPORTANTE: SI ESTO NO ESTÁ EN TRUE, NO ENVÍA NADA
  mqtt_time_send = true;
...
void settingsReset() {
  ...
  mqtt_time_send = true; // Aseguramos que esté activo aquí también
  strlcpy(mqtt_user, "plcuser", sizeof(mqtt_user));
  strlcpy(mqtt_password, "plc", sizeof(mqtt_password));
  strlcpy(mqtt_server, "10.110.191.56", sizeof(mqtt_server));
  strlcpy(mqtt_cloud_id, "ESP32_S3_ANGEL_VALOR", sizeof(mqtt_cloud_id));
